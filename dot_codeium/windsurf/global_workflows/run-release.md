...
High-level workflow สำหรับ auto-detect และ release ไปยัง npm, crates, vscode, webstore, docker, pkg.pr.new
...
7. ตรวจสอบ `.github/workflows/publish.yml` มีอยู่ → รองรับ `pkg.pr.new`
...
6. ถ้า detect `pkg.pr.new`: ทำ `/follow-pkg-new`
...
- ตรวจสอบ `.github/workflows/publish.yml` สำหรับ pkg.pr.new
...
- ใช้ `/follow-pkg-new` สำหรับ pkg.pr.new
